    ...
    with open(nombre_fichero, 'w') as f:
        ...
        for caso in range(num_casos):
            ...
            for prenda in range(num_prendas):
                ...
                # Asegúrate de que el rango de modelos sea válido
                min_modelos = max(modelos_minimos_por_prenda, rango_modelos[0])
                max_modelos = min(modelos_minimos_por_prenda, rango_modelos[1])

                # Evita un rango vacío, ajustando si es necesario
                if min_modelos > max_modelos:
                    max_modelos = min_modelos

                # Ahora, genera un número aleatorio dentro del rango ajustado
                num_modelos = random.randint(min_modelos, max_modelos) if prenda < num_prendas - 1 else modelos_restantes
...
generar_fichero_entrada_progresivo(
    nombre_fichero="casos_grandes.in",
    num_casos=100,  # Aumentamos el número de casos
    rango_presupuesto=(5000, 15000),  # Aumento del presupuesto
    rango_prendas=(5, 50),  # Aumento del número de prendas
    rango_modelos=(100, 1000),  # Rango de modelos más grande
